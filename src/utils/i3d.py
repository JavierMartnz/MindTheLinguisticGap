...
from src.utils.dsbn import DomainSpecificBatchNorm3d
...
class Unit3D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv3d(x)# tj : before [1,1024,1,1,1] -> after [1,2000,1,1,1]
        if self._use_batch_norm:
            x = self.bn(x)
...
class InceptionI3d(nn.Module):
    ...
    def __init__(
        ...
        self.logits = Unit3D(
            in_channels=384 + 384 + 128 + 128, # tj : =1024
            output_channels=self._num_classes, # tj : =2000
            kernel_shape=[1, 1, 1],
            padding=0,
            activation_fn=None,
    ...
    def forward(self, x): # tj : x - torch.Size([1, 3, 64, 224, 224])
        for end_point in self.VALID_ENDPOINTS:
            if end_point in self.end_points:
                x = self._modules[end_point](x)
        # [batch x featuredim x 1 x 1 x 1]
        # tj : x - torch.Size([1, 1024, 8, 7, 7])
        embds = self.dropout(self.avgpool(x)) # tj : embds - torch.Size([1, 1024, 1, 1, 1])

        # [batch x classes x 1 x 1 x 1]
        x = self.logits(embds) # tj : x - torch.Size([1, 2000, 1, 1, 1])
