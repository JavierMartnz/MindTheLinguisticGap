...
class InceptionI3d(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True, final_endpoint='Logits', name='inception_i3d',
                 in_channels=3, dropout_keep_prob=0.5, window_size=64, input_size=256, final_pooling_size=1024):
        ...
        # HERE CHANGES THE DIMENSIONALITY
        self.final_pooling = nn.AvgPool3d(kernel_size=[1, 1, final_pooling_size], stride=[1, 1, final_pooling_size])
    ...
    def replace_logits(self, num_classes, final_pooling_size):
        self._num_classes = num_classes
        self.logits = Unit3D(in_channels=1024//final_pooling_size, output_channels=self._num_classes,
                             kernel_shape=[1, 1, 1],
                             padding=0,
                             activation_fn=None,
                             use_batch_norm=False,
    ...
    def forward(self, x):
        ...
        x = self.avg_pool(x)
        x = self.final_pooling(x.permute(0, 4, 2, 3, 1), ).permute(0, 4, 2, 3, 1)
        x = self.dropout(x)
        x = self.logits(x)
