...
# hotfix for int_classes not importing
int_classes = int
...
# https://www.programmersought.com/article/86531627156/
def my_collate(batch):
    "Puts each data field into a tensor with outer dimension batch size"
    batch = list(filter(lambda x: x is not None, batch))  # tj : fitler out the None in __getitem__
    if not batch:  # tj : batch is empty list
        return None
