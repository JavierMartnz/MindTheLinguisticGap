...
def text_on_image_signbank(img, txt, pos):
    x = 5
    y = 5
    img_pil = Image.fromarray(img)
    draw = ImageDraw.Draw(img_pil)
...
def text_on_image_bbc(img, pre, t, sub=None):
    x = 5
    y = 5
    img_pil = Image.fromarray(img)
    draw = ImageDraw.Draw(img_pil)
    ...
    draw.text((5, 180), 'pre', fill=(255, 255, 255), font=font)
    draw.rectangle((30, 180, 30+180, 180+10), fill='white')
    range_size = pre.shape[0]
    line_points = []
    for i in range(range_size-1):
        # if gt[i]:
        #     draw.rectangle(( 30 + i / range_size * 180, 180, 30+ (i+1) / range_size * 180, 180+10), fill='#ff0000')
        if pre[i] > 0.5:
            draw.rectangle((30 + i / range_size * 180, 180, 30 + (i + 1) / range_size * 180, 180 + 10), fill='#00ff00')
        # tj : draw pre curve
        line_points.append((30 + i / range_size * 180, 180 + (1-pre[i]) * 10))
    ...
    # tj : draw cursor
    draw.line((30 + t / range_size * 180, 180, 30 + t / range_size * 180, 190), fill=0, width=1)
    ...
    if sub is not None:
        ...
        y = 200
        font_name = "DejaVuSans-Bold.ttf"
        font = ImageFont.truetype(font_name, int(img.shape[0] / 30))
        w, h = font.getsize(sub)


        for line in textwrap.wrap(sub, 60): # tj : 60 is the nubmer of letters
            draw.rectangle((x, y, x + w, y + h), fill="black")
            draw.text((x, y), line, fill=(255, 255, 255), font=font)
            y += h

    return np.asarray(img_pil)
def text_on_image(img, txt, gt, pre, t):
    x = 5
    y = 5
    img_pil = Image.fromarray(img)
    draw = ImageDraw.Draw(img_pil)
    ...
    draw.rectangle((5, 180, 5 + w, 180 + h), fill="black")
    draw.text((5, 180), 'gt', fill=(255, 255, 255), font=font)
    draw.rectangle((30, 180, 30 + 180, 180 + 10), fill='white')

    w, h = font.getsize('pre')
    draw.rectangle((5, 200, 5 + w, 200 + h), fill="black")
